part of 'guess_second_citizenship_screen.dart';

const _kDefaultRewardValue = 1;

class GuessSecondCitizenshipScreenPresenter extends StatefulWidget {
  static GuessSecondCitizenshipScreenPresenterState of(BuildContext context) {
    return context.findAncestorStateOfType<GuessSecondCitizenshipScreenPresenterState>()!;
  }

  final Widget child;

  const GuessSecondCitizenshipScreenPresenter({required this.child, super.key});

  @override
  State<GuessSecondCitizenshipScreenPresenter> createState() => GuessSecondCitizenshipScreenPresenterState();
}

class GuessSecondCitizenshipScreenPresenterState extends State<GuessSecondCitizenshipScreenPresenter>
  ...
  void loadRandomPlayers() {
    if (mounted) {
      ...
      context.read<RandomFootballPlayersBloc>().add(
        RandomFootballPlayersEventGet(count: 1, withSecondCitizenship: true),
      );
    }
  }
